...
void RenderingSystem::GeometryPass(const std::vector<RenderObject> &objects) {
    ...
    for (const RenderObject &obj : objects) {
        ...
        const float wireframe_flag = (render_mode_ == RenderMode::Wireframe) ? 1.0f : 0.0f;
        cb.displacement_params = {displacement_scale_, normal_displacement_scale_, wireframe_flag, 0.0f};
